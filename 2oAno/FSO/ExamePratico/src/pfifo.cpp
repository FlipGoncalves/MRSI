...
void init_pfifo(PriorityFIFO* pfifo)
{
   ...
   cond_init(&pfifo->is_empty, NULL);
   cond_init(&pfifo->is_full, NULL);
}
...
void term_pfifo(PriorityFIFO* pfifo)
{
   ...
   cond_destroy(&pfifo->is_empty);
   cond_destroy(&pfifo->is_full);
}
...
void insert_pfifo(PriorityFIFO* pfifo, int id, int priority)
{
   ...
   {
      cond_wait(&pfifo->is_full, &pfifo->access);
   }
...
int retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   {
      cond_wait(&pfifo->is_empty, &pfifo->access);
   }
