...
typedef struct
{
   ...
   pthread_cond_t patients_conds[MAX_PATIENTS];
} HospitalData;

HospitalData * hd = NULL;

// TODO point: if necessary, add module variables here
int ta_patient[MAX_PATIENTS];
int ta_doctors[MAX_DOCTORS];
int ta_nurses[MAX_NURSES];
...
void term_simulation(int np) {
   ...
   mutex_destroy(&hd->access);
   for (int i = 0; i < np; i++)
      cond_destroy(&hd->patients_conds[i]);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   {
      cond_wait(&hd->patients_conds[id], &hd->access);
   }
...
void doctor_life(int id)
{
   while (true)
   {
      if (!doctor_iteration(id))
         break;
   }
}

void nurse_life(int id)
{
   while (true)
   {
      if (!nurse_iteration(id))
         break;
   }
}
...
int main(int argc, char *argv[])
{
   ...
   pthread_t thr_patients[MAX_PATIENTS], thr_doctors[MAX_DOCTORS], thr_nurses[MAX_NURSES];

   for (int i = 0; i < npatients; i++)
   {
      ta_patient[i] = i;
      thread_create(&thr_patients[i], NULL, patient_thread, (void *)&ta_patient[i]);
   }

   for (int i = 0; i < ndoctors; i++)
   {
      ta_doctors[i] = i;
      thread_create(&thr_doctors[i], NULL, doctor_thread, (void *)&ta_doctors[i]);
   }
   for (int i = 0; i < nnurses; i++)
   {
      ta_nurses[i] = i;
      thread_create(&thr_nurses[i], NULL, nurse_thread, (void *)&ta_nurses[i]);
   }
   ...
   for (int i = 0; i < ndoctors; i++)
   {
      thread_join(thr_doctors[i], NULL);
   }
